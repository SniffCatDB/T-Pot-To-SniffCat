...
const { NETCATDB_API_KEY, SERVER_ID, DEBUG_MODE, DISCORD_WEBHOOKS_ENABLED, DISCORD_WEBHOOKS_URL } = config.MAIN;
...
const sendBulkReport = async () => {
	...
	try {
		...
		const { data } = await axios.post('http://127.0.0.1:4070/api/v1/report', form, {
			headers: {
				Key: NETCATDB_API_KEY,
				...form.getHeaders(),
			},
		});
...
const reportToAbuseIPDb = async (honeypot, { srcIp, dpt = 'N/A', service = 'N/A', timestamp }, categories, comment) => {
	...
	try {
		...
		saveReportedIPs();
		log(0, `${honeypot} -> ✅ Reported ${srcIp} [${dpt}/${service}] | Categories: ${categories} | Score: ${res.abuseConfidenceScore}%`);
	} catch (err) {
